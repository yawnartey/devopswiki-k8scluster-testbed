...
# echo 'KUBELET_EXTRA_ARGS=--node-labels=role=backend' > /etc/sysconfig/kubelet
...
# KUBECONFIG=/tmp/admin.conf kubectl label node $(hostname -f) node-role.kubernetes.io/backend=
